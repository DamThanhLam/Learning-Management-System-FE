...
export default function ShoppingCart() {
  ...
  const handleRemove = (courseId: string) => {
    ...
    fetch(BASE_URL_COURSE_SERVICE + '/shopping-cart', {
      ...
      .then((res) => res.json())
      .then((data) => {
        console.log('DELETE response:', data) // Log the actual response object
      })
      .catch((err) => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cartItems.length === 0 ? (
            <div className="text-center text-gray-500 py-12">You don't have any course in your cart yet</div>
          ) : (
            cartItems.map((item: any) => (
              <div key={item.id} className="flex items-center border p-4 rounded-lg">
                <img src={item.urlAvt} alt={`${item.courseName} Thumbnail`} className="w-24 h-24 object-cover rounded-lg" />
                <div className="ml-4 flex-1">
                  <h2 className="text-lg font-semibold">{item.courseName}</h2>
                  <p className="text-sm text-gray-500">By {item.teacherName}</p>
                  <p className="text-sm text-gray-500">
                    {item.countLectures} Lectures, <span className="text-yellow-600">Rated {item.totalReview.toFixed(1)} ★</span>
                  </p>
        ...
        <div className="border p-4 rounded-lg">
          ...
          <button
            className="w-full mt-4 bg-blue-500 text-white py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
